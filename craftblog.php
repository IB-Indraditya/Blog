    ...
    <section class='main' id="gotop">
        ...
        <div class="container">
            ...
            <?php
                ...
                $sql = "SELECT blogcraft.*,
                            blogregister.fullname,
                            (SELECT COUNT(*) FROM blog_likes WHERE blog_likes.post_id = blogcraft.id) AS like_count,
                            (SELECT COUNT(*) FROM blog_likes WHERE blog_likes.post_id = blogcraft.id AND blog_likes.user_email = ?) AS user_liked
                        FROM blogregister
                        INNER JOIN blogcraft ON blogcraft.author_email = blogregister.email
                        ORDER BY blogcraft.created_at DESC";
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        $commentSql = "SELECT * FROM comments3 WHERE post_id = $postId ORDER BY created_at ASC";
            ...
            <div class="add-post" id='toolbar'>
                <a href="dashboard.php">Add New Post</a>
            </div>
        </div>
